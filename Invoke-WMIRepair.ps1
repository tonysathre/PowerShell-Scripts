            ...
            try {
                ...
                function RegisterMof([System.IO.FileSystemInfo]$item)
                {
                   ...
                   else
                   {
                      ...
                      {
                        $register = $false
                        Write-Host "Skipping - file is empty: $($item.FullName)"
                        return
                      }
                ...
                [string]$CCMmof = Join-Path $env:windir "CCM"
                [System.IO.FileSystemInfo[]]$itemlist = Get-ChildItem $wbempath, $CCMmof -Recurse | Where-Object { $_.FullName.Contains('AutoRecover') -ne $true}
                ...
                Write-Error "Error at line number: $($_.InvocationInfo.ScriptLineNumber)"
